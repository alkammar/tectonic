 ...
 * If a precondition or sub is executed it will call the {@link #map(Class, TectonicEvent)} and {@link #observe(TectonicEvent, TectonicEvent, UseCase)}
 * method first before a call is made to any version of {@link #trigger(TectonicEvent, ResultActor, TectonicEvent)}.
 * So make sure that you do the all use cases executions inside {@link #trigger(TectonicEvent, ResultActor, TectonicEvent)}
...
public interface Triggers<E extends TectonicEvent> {
     ...
     * executing a use case corresponding to the passed {@code event}, specifying the {@code the resultActor} (if any)
     * and the {@code} (if any). You should handle any null parameter as they are all allowed.
    ...
    /**
     * Version of {@link #trigger(TectonicEvent, ResultActor, TectonicEvent)} but
     ...
     * This is followed by a call to {@link #trigger(TectonicEvent, ResultActor, TectonicEvent)},
